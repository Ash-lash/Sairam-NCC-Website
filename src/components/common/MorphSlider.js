...
void main() {
  ...
  // Render Backgrounds (Darkened heavily)
  vec3 bgC = vec3(
    texture2D(tCurrent, sC_cover + vec2(ca, 0.0)).r,
    texture2D(tCurrent, sC_cover).g,
    texture2D(tCurrent, sC_cover - vec2(ca, 0.0)).b
  ) * 0.35; // 35% brightness
  
  vec3 bgN = vec3(
    texture2D(tNext, sN_cover + vec2(ca, 0.0)).r,
    texture2D(tNext, sN_cover).g,
    texture2D(tNext, sN_cover - vec2(ca, 0.0)).b
  ) * 0.35;

  // Render Foregrounds
  vec3 fgC = vec3(
    texture2D(tCurrent, sC_contain + vec2(ca, 0.0)).r,
    texture2D(tCurrent, sC_contain).g,
    texture2D(tCurrent, sC_contain - vec2(ca, 0.0)).b
  );
  
  vec3 fgN = vec3(
    texture2D(tNext, sN_contain + vec2(ca, 0.0)).r,
    texture2D(tNext, sN_contain).g,
    texture2D(tNext, sN_contain - vec2(ca, 0.0)).b
  );

  // Mask foregrounds to their aspect ratio bounds with a smooth edge
  float edgeDistC = min(min(sC_contain.x, 1.0 - sC_contain.x), min(sC_contain.y, 1.0 - sC_contain.y));
  float alphaC = smoothstep(0.0, 0.003, edgeDistC);
  
  float edgeDistN = min(min(sN_contain.x, 1.0 - sN_contain.x), min(sN_contain.y, 1.0 - sN_contain.y));
  float alphaN = smoothstep(0.0, 0.003, edgeDistN);

  vec3 colC = mix(bgC, fgC, alphaC);
  vec3 colN = mix(bgN, fgN, alphaN);
...
class MorphEngine {
  ...
  loadTextures() {
    this.items.forEach((item, index) => {
      ...
      img.onerror = () => {
        // If the WebP URL 404s (e.g. extension hasn't generated it yet), fallback to original
...
export default function MorphSlider({
  ...
  useEffect(() => {
    if (!autoplay || hovering || items.length === 0) return undefined;
